import { DuckDuckGoImage, imageNextSearch, imageSearch, ImageSearchResult } from "../utils/search";
...
export async function searchImage({ query, cursor, signal, layout }: SearchImageParams): Promise<ImageSearchResult> {
    ...
    try {
        if (cursor) {
            return await imageNextSearch(cursor.next, cursor.vqd, MAX_RETRIES, signal)
        }
        return await imageSearch(query, { moderate, filters: {layout} }, MAX_RETRIES, signal)
    } catch (err: any) {
...
export async function downloadImage({ image, image_token }: DuckDuckGoImage, showToastMessage: boolean = true): Promise<string> {
