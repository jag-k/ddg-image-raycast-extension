...
async function makeNextFromQuery(query: string, options: ImageSearchOptions = {}): Promise<{next: string, vqd: string}> {
    let vqd = options.vqd!;
...
export async function imageSearch(query: string, options: ImageSearchOptions = {}, retries: number = MAX_RETRIES, signal?: AbortSignal): Promise<ImageSearchResult> {
    const { next, vqd } = await makeNextFromQuery(query, options)
    return await imageNextSearch(next, vqd, retries, signal);
}

export async function imageNextSearch(next: string, vqd: string, retries: number = MAX_RETRIES, signal?: AbortSignal): Promise<ImageSearchResult> {
